...
class WideAndDeep(nn.Module):
    def __init__(self, in_dim: int, deep_hidden: Tuple[int, ...] = (128, 64), dropout: float = 0.2):
        super().__init__()
        # Wide part: simple linear model on raw features
...
def train(args):
    out_dir = os.path.join("outputs", "widedeep")
    ensure_dir(out_dir)
    ...
    model = WideAndDeep(in_dim=X_tr.shape[1], deep_hidden=tuple(args.hidden), dropout=args.dropout).to(device)
    ...
    # Save artifacts
    ts = int(time.time())
    ckpt_path = os.path.join(out_dir, f"widedeep_{ts}.pt")
    torch.save({"state_dict": model.state_dict(), "feature_names": splits.feature_names}, ckpt_path)

    plot_and_save_curves(yt, pt, out_dir, prefix="widedeep")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train Wide & Deep model for cheat detection")
